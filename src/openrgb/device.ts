...
export class DeviceData {
    ...
    static parse(data) {
        ...
        try {
            ...
            const hexString = Array.from(debugView).map(b => b.toString(16).padStart(2, '0')).join(' ');
            const asciiString = Array.from(debugView).map(b => (b >= 32 && b <= 126) ? String.fromCharCode(b) : '.').join('');
            ...
            console.log(`OpenRGB: Successfully parsed device: ${device.name} with ${device.leds.length} LEDs`);
            return device;
            ...
            if (parser.offset > 0) {
                ...
                const contextHex = Array.from(contextView).map(b => b.toString(16).padStart(2, '0')).join(' ');
