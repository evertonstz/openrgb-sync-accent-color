...
export class BinaryParser {
    ...
    readUint32() {
        if (this.offset + 4 > this.data.byteLength) {
            throw new Error(`Cannot read Uint32 at offset ${this.offset}, buffer size: ${this.data.byteLength}`);
        }
    ...
    readUint16() {
        if (this.offset + 2 > this.data.byteLength) {
            throw new Error(`Cannot read Uint16 at offset ${this.offset}, buffer size: ${this.data.byteLength}`);
        }
    ...
    readString() {
        ...
        if (this.offset + length > this.data.byteLength) {
            throw new Error(`Cannot read string of length ${length} at offset ${this.offset}, buffer size: ${this.data.byteLength}`);
        }
    ...
    readRGBColor() {
        if (this.offset + 4 > this.data.byteLength) {
            throw new Error(`Cannot read RGBColor at offset ${this.offset}, buffer size: ${this.data.byteLength}`);
        }
    ...
    skip(bytes) {
        if (this.offset + bytes > this.data.byteLength) {
            throw new Error(`Cannot skip ${bytes} bytes at offset ${this.offset}, buffer size: ${this.data.byteLength}`);
        }
        this.offset += bytes;
    }
